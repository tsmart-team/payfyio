...
> `2D` = direct (non-secure) card charge via `createPayment`. PayTR, PayPal and
> Coinbase only support the secure/approval/hosted flow (`initThreeDSPayment`).
> Coinbase is crypto checkout — BTC, ETH, USDC, etc., via Coinbase Commerce's
> hosted page. Refunds on Coinbase Commerce must be issued from its dashboard.
...
```typescript
import { Payfyio, consoleNotifier, webhookNotifier } from 'payfyio';
