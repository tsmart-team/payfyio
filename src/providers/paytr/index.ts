...
export class PayTR extends PaymentProvider {
  ...
  constructor(config: PaymentProviderConfig & { merchantId: string; merchantSalt: string }) {
    ...
    // PayTR signs every payload with HMAC keyed by merchant_key (passed as
    // apiKey). Without it, request tokens and callback verification cannot
    // be computed, so fail fast rather than producing invalid signatures.
    if (!config.apiKey) {
      throw new Error('Merchant Key (apiKey) is required');
    }
    ...
    this.setupAxiosSecurity(this.client, 'paytr');
  }
...
<body>
  ...
  async completeThreeDSPayment(callbackData: PayTRCallbackData): Promise<PaymentResponse> {
    try {
      ...
      if (!isValid) {
        // Strongest abuse signal: someone is POSTing a 3DS callback whose
        // signature doesn't match — i.e. trying to forge a "paid" result.
        this.emitSecurityEvent(
          'callback_verification_failed',
          'PayTR callback failed hash verification',
          {
            severity: 'critical',
            metadata: {
      ...
      return {
        ...
        // Only surface the provider's response body — never the full axios
        // error, whose `config.headers` carries the Authorization secret.
        rawResponse: error.response?.data,
      };
    }
  }
  ...
  async _cancelWithAmount(request: CancelRequest & { price: string }): Promise<CancelResponse> {
    try {
      ...
      return {
        ...
        // Only the provider response body — not the axios error with headers.
        rawResponse: error.response?.data,
      };
    }
  }
  ...
  async binCheck(binNumber: string): Promise<BinCheckResponse> {
    try {
      const token = generateBinDetailToken(this.merchantId, binNumber, this.merchantSalt, this.merchantKey);
  ...
  async installmentInfo(request: InstallmentInfoRequest): Promise<InstallmentInfoResponse> {
    try {
      const token = generateBinDetailToken(this.merchantId, request.binNumber, this.merchantSalt, this.merchantKey);
      const params: Record<string, string> = {
        merchant_id: this.merchantId,
        merchant_key: this.merchantKey,
        merchant_salt: this.merchantSalt,
