...
import {
  ...
  IyzicoPostAuthResponse,
  IyzicoSubmerchantResponse,
} from './types';
import { toMinor, formatMinor } from '../../core/money';
...
export class Iyzico extends PaymentProvider {
  ...
  /**
   * iyzico'nun ayrı bir idempotency header'ı yoktur; dedup `conversationId`
   * üzerinden yapılır (iyzico bu değeri yanıtta aynen döndürür). Çağıran bir
   * `idempotencyKey` verip `conversationId` vermediyse, key'i conversationId
   * olarak kullanırız — böylece idempotency niyeti iyzico'nun gerçek dedup
   * alanına akar. `conversationId` zaten verilmişse ona dokunmayız.
   */
  private resolveConversationId(conversationId?: string, idempotencyKey?: string): string | undefined {
  ...
  private mapToIyzicoRequest(request: PaymentRequest): IyzicoPaymentRequest {
    return {
      ...
      // Marketplace split: iyzico bölünmeyi kalem-bazlı taşır (subMerchantKey +
      // subMerchantPrice). split[i] → basketItems[i] eşlemesi yapılır; platform
      // komisyonu, item price ile subMerchantPrice farkında kalır.
      basketItems: request.basketItems.map((item, i) => {
        const split = request.split?.[i];
        return {
          id: item.id,
          name: item.name,
          ...
          ...(split
            ? {
                subMerchantKey: split.submerchantId,
                subMerchantPrice: formatMinor(split.amountMinor, request.currency as string),
              }
            : {}),
        };
      }),
  ...
  async createPayment(request: PaymentRequest): Promise<PaymentResponse> {
    try {
      const iyzicoRequest = {
        ...
        conversationId: this.resolveConversationId(request.conversationId, request.idempotencyKey),
      };
      // capture === false → sadece provizyon al (pre-auth). Sonradan
      // capturePayment ile çekilir veya voidAuthorization ile serbest bırakılır.
      const endpoint = request.capture === false ? '/payment/preauth' : '/payment/auth';
  ...
  async initThreeDSPayment(request: ThreeDSPaymentRequest): Promise<ThreeDSInitResponse> {
    try {
      ...
      // capture === false → 3DS provizyon (pre-auth) akışı. Callback'ten sonra
      // completeThreeDSPayment provizyonu oluşturur; çekim capturePayment ile.
      const endpoint =
  ...
  /**
   * Capture (PostAuth) — bir pre-auth (`capture: false`) ile tutulan tutarın
   * tamamını veya bir kısmını çeker.
   *
   * Kısmi capture için `amountMinor` (+ `currency`) verin; verilmezse provizyonun
   * tamamı çekilir. iyzico kısmi capture'da `paidPrice` alanını bekler.
   */
  async capturePayment(request: CaptureRequest): Promise<CaptureResponse> {
    try {
      const currency = (request.currency as string) || 'TRY';
      const paidPrice =
        request.amountMinor != null
          ? formatMinor(request.amountMinor, currency)
          : undefined;

      const response = await this.sendRequest<IyzicoPostAuthResponse>('/payment/postauth', {
        locale: this.config.locale || 'tr',
        conversationId: this.resolveConversationId(request.conversationId, request.idempotencyKey),
        paymentId: request.paymentId,
        // Tam capture'da paidPrice gönderilmez → iyzico provizyonun tamamını çeker.
        paidPrice,
        ip: request.ip,
      });

      const capturedAmountMinor =
        response.paidPrice != null
          ? toMinor(response.paidPrice, response.currency || currency)
          : request.amountMinor;
  ...
  /**
   * Void — henüz capture edilmemiş bir pre-auth'u serbest bırakır. iyzico'da
   * ayrı bir void uç noktası yoktur; capture edilmemiş provizyon `/payment/cancel`
   * ile serbest bırakılır (aynı gün iptaliyle aynı uç nokta).
   */
  ...
  /**
   * ===================
   * SUBMERCHANT (Marketplace) METHODS
   * ===================
   */

  /** Ortak: PayfyioSubmerchant alanlarını iyzico submerchant gövdesine çevirir. */
  private mapSubmerchantBody(request: SubmerchantCreateRequest | SubmerchantUpdateRequest): Record<string, unknown> {
    return {
      locale: this.config.locale || 'tr',
      conversationId: request.conversationId,
      subMerchantExternalId: request.externalId,
      subMerchantType: request.type,
      address: request.address,
      // İşletme adı (name) ile yetkili kişi (contactName/Surname) ayrıdır.
      // iyzico PERSONAL/PRIVATE_COMPANY alt-satıcılarda contactName zorunlu.
      ...
      // Güncellemede provider referansı
      subMerchantKey: (request as SubmerchantUpdateRequest).submerchantKey,
    };
  }

  private mapSubmerchantResponse(response: IyzicoSubmerchantResponse): SubmerchantResponse {
    return {
      status: this.mapStatus(response.status),
      submerchantKey: response.subMerchantKey,
      externalId: response.subMerchantExternalId,
      conversationId: response.conversationId,
      errorCode: response.errorCode,
      errorMessage: response.errorMessage,
      rawResponse: response,
  ...
  async createSubmerchant(request: SubmerchantCreateRequest): Promise<SubmerchantResponse> {
    try {
      const response = await this.sendRequest<IyzicoSubmerchantResponse>(
        '/onboarding/submerchant',
        this.mapSubmerchantBody(request),
      );
      return this.mapSubmerchantResponse(response);
    } catch (error: any) {
      return {
        status: PaymentStatus.FAILURE,
        errorMessage: error.message || 'Submerchant create failed',
        rawResponse: error.response?.data,
      };
    }
  }
  ...
  async updateSubmerchant(request: SubmerchantUpdateRequest): Promise<SubmerchantResponse> {
    try {
      ...
      return {
        status: PaymentStatus.FAILURE,
        errorMessage: error.message || 'Submerchant update failed',
        rawResponse: error.response?.data,
      };
    }
  }
  ...
  async retrieveSubmerchant(externalId: string): Promise<SubmerchantResponse> {
    try {
      ...
      return {
        status: PaymentStatus.FAILURE,
        errorMessage: error.message || 'Submerchant retrieve failed',
        rawResponse: error.response?.data,
      };
    }
  }
