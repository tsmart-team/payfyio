...
/**
 * PostAuth (capture) yanıtı — bir pre-auth ile tutulan tutarın çekilmesi.
 */
export interface IyzicoPostAuthResponse extends IyzicoResponse {
  paymentId?: string;
  /** Çekilen tutar (kısmi capture'da istenen kadar). */
  paidPrice?: number;
  price?: number;
  currency?: string;
}
...
/**
 * iyzico submerchant (marketplace alt-satıcı) yanıtı.
 */
export interface IyzicoSubmerchantResponse extends IyzicoResponse {
...
export interface IyzicoBasketItem {
  ...
  itemType: string;
  price: string;
  /** Marketplace split — bu kalemin gelirinin gideceği alt-satıcı. */
  subMerchantKey?: string;
  /** Alt-satıcıya gidecek tutar (decimal string); platform komisyonu farkta kalır. */
