...
export class Parampos extends PaymentProvider {
  ...
  async completeThreeDSPayment(
    ...
    try {
      // Verify callback hash. CRITICAL: the GUID used to recompute the hash
      // must come from our server-side config — never from callbackData.
      // The hash secret cannot be a value the caller also supplies, or the
      // signature is trivially forgeable (set mdStatus='1' + a matching hash).
