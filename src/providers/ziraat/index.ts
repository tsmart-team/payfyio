...
export class Ziraat extends PaymentProvider {
  ...
  async completeThreeDSPayment(callbackData: any): Promise<PaymentResponse> {
    ...
    // SECURITY (fail-closed): NestPay 3D response HASH (HMAC-SHA512 over the
    // param list + storeKey) must be recomputed and constant-time compared
    // before trusting mdStatus / ProcReturnCode. Not implemented yet, so we
    // refuse to confirm — a forged callback would otherwise mark an unpaid
    // order paid. Do NOT remove until response-hash verification + amount
    // binding are implemented and tested against the bank sandbox.
    return {
      status: PaymentStatus.FAILURE,
      paymentId: orderId,
      errorMessage:
        'Ziraat (NestPay) 3DS callback hash verification is not implemented; refusing to confirm payment. ' +
