...
export class Garanti extends PaymentProvider {
  ...
  async initThreeDSPayment(request: ThreeDSPaymentRequest): Promise<ThreeDSInitResponse> {
    return this.build3DForm(request);
  }

  /**
   * GVP "Sabit Tutarlı Tekrarlı Satış" — the 3D form plus five plan fields.
   * The bank charges the remaining instalments itself on the schedule below;
   * only the first one goes through 3D Secure.
   *
   * The plan fields are deliberately NOT part of `secure3dhash` — the bank's
   * signed field list is fixed and does not include them.
   */
  async initRecurringPayment(request: RecurringPaymentRequest): Promise<ThreeDSInitResponse> {
    const { recurring } = request;
    if (!recurring || !Number.isInteger(recurring.totalPayments) || recurring.totalPayments < 1) {
      return {
        status: PaymentStatus.FAILURE,
        errorMessage: 'recurring.totalPayments must be a positive integer',
      };
    }
    return this.build3DForm(request, recurring);
  }
  ...
  private build3DForm(
    ...
    const fields: Record<string, string> = {
      ...
      // Kart alanları yoksa hiç gönderilmez — GVP o zaman kendi "Ortak Ödeme"
      // sayfasını açıp kartı müşteriden kendisi alır. Boş string GÖNDERMEK
      // aynı şey değildir: banka onu geçersiz kart sayar.
    ...
    if (recurring) {
      const FREQ = { daily: 'D', weekly: 'W', monthly: 'M' } as const;
      fields.recurringtype = 'R';
      fields.totalpaymentnum = String(recurring.totalPayments);
      fields.frequencytype = FREQ[recurring.frequency];
      fields.frequencyinterval = String(recurring.interval ?? 1);
      fields.startdate = formatGarantiDate(recurring.startDate);
    }
    return {
