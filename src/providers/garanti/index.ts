...
export class Garanti extends PaymentProvider {
  ...
  async completeThreeDSPayment(callbackData: any): Promise<PaymentResponse> {
    ...
    // SECURITY (fail-closed): the Garanti GVP 3D response carries a HASH /
    // HASHPARAMS that MUST be recomputed with secure3DStoreKey and compared
    // before trusting mdstatus. That verification is not implemented yet, so
    // we refuse to report SUCCESS — otherwise a forged callback (mdstatus=1)
    // would mark an unpaid order as paid. Do NOT remove this guard until
    // response-hash verification + amount binding are implemented and tested
    // against the bank's sandbox.
    return {
      status: PaymentStatus.FAILURE,
      paymentId: orderId,
      errorMessage:
        'Garanti 3DS callback hash verification is not implemented; refusing to confirm payment. ' +
        'Implement response-HASH verification before enabling this provider in production.',
      rawResponse: callbackData,
    };
  }
