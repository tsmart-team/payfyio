...
/** GVP `startdate` — YYYYMMDD. Defaults to today when no date is given. */
export function formatGarantiDate(value?: Date | string): string {
  if (typeof value === 'string') {
    const digits = value.replace(/\D/g, '');
    if (digits.length === 8) return digits;
    const parsed = new Date(value);
    if (isNaN(parsed.getTime())) throw new Error(`Invalid Garanti startDate: ${value}`);
    return formatGarantiDate(parsed);
  }
  const d = value ?? new Date();
  if (isNaN(d.getTime())) throw new Error('Invalid Garanti startDate');
  return (
    `${d.getFullYear()}` +
