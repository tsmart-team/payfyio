...
/**
 * Tekrarlı (recurring) ödeme planı — bankanın kendi tekrarlı satış ürününe
 * kurulur; ilk çekim 3DS ile doğrulanır, kalanları **banka** otomatik provizyona
 * gönderir. Kart saklamaya veya kendi zamanlayıcına gerek yoktur.
 *
 * Stripe aboneliğinden farkı: seri **sonludur** (`totalPayments`) ve tutar seri
 * kurulurken sabitlenir — süresiz yenileme ve plan içi tutar değişimi yoktur.
 */
export interface RecurringPlan {
  /** İlk çekim dahil toplam çekim adedi. Garanti `totalpaymentnum`. */
  totalPayments: number;
  /** Çekim sıklığı. Garanti `frequencytype` (M/W/D). */
  frequency: 'daily' | 'weekly' | 'monthly';
  /** Kaç periyotta bir çekileceği. Varsayılan 1. Garanti `frequencyinterval`. */
  interval?: number;
  /** İlk çekim tarihi. Verilmezse bugün. Garanti `startdate` (YYYYMMDD). */
  startDate?: Date | string;
}

/**
 * Tekrarlı ödeme başlatma isteği — 3DS isteğinin üstüne planı ekler.
 */
export interface RecurringPaymentRequest extends ThreeDSPaymentRequest {
...
export interface SubmerchantCreateRequest {
  ...
  /** İşletme/mağaza adı (iyzico `name`). */
  name: string;
  email: string;
  /** Bizim tarafımızdaki kalıcı kimlik (carrier id); idempotent eşleme için. */
  externalId: string;
  /**
   * Yetkili kişinin adı. iyzico PERSONAL/PRIVATE_COMPANY alt-satıcılarda
   * **zorunlu** (`contactName`). İşletme adından (`name`) ayrıdır.
   */
  contactName?: string;
  /** Yetkili kişinin soyadı (iyzico `contactSurname`). */
