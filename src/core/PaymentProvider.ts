...
export abstract class PaymentProvider {
  ...
  /**
   * Capture (post-auth) — bir pre-auth (`capture: false`) ile tutulan tutarı
   * çeker. Override edilmezse desteklenmez. Karşılığı: iyzico PostAuth, Stripe
   * manual capture.
   */
  async capturePayment(_request: CaptureRequest): Promise<CaptureResponse> {
    throw new Error('Capture not supported by this provider');
  }

  /**
   * Tekrarlı ödeme serisi başlatır — ilk çekim 3DS ile doğrulanır, kalanlarını
   * banka kendi takvimine göre otomatik provizyona gönderir. Override
   * edilmezse desteklenmez.
   *
   * Bilerek `initThreeDSPayment` üstünde opsiyonel bir alan DEĞİL ayrı bir
   * metot: sessizce yok sayılan bir plan, abonelik kurduğunu sanıp tek çekim
   * yapmak demek olurdu. Desteklemeyen sağlayıcıda bu çağrı hata verir.
   *
   * Dönüşü `initThreeDSPayment` ile aynıdır; seriyi tamamlamak için yine
   * `completeThreeDSPayment` çağrılır. Sonraki çekimler için callback
   * gelmeyebilir — durumu `getPayment(orderId)` ile sorgulayın.
   */
  async initRecurringPayment(
    _request: RecurringPaymentRequest,
  ): Promise<ThreeDSInitResponse> {
    throw new Error('Recurring payments are not supported by this provider');
  }

  /**
   * Void — henüz capture edilmemiş bir pre-auth'u serbest bırakır. Override
   * edilmezse desteklenmez. Capture edilmiş ödeme için `refund` kullanın.
   */
  async voidAuthorization(
    _request: VoidAuthorizationRequest,
  ): Promise<VoidAuthorizationResponse> {
    throw new Error('Void not supported by this provider');
  }

  /**
   * Payout / Transfer — para-OUT, 3. tarafa. Override edilmezse desteklenmez.
   * Karşılığı: Stripe transfers/payouts, iyzico submerchant payout. Gerçek para
   * çıkışı custody/lisans gerektirir (bkz. PSP yol haritası).
   */
  async payout(_request: PayoutRequest): Promise<PayoutResponse> {
    throw new Error('Payout not supported by this provider');
  }

  /**
   * Alt-satıcı oluştur (marketplace). Override edilmezse desteklenmez.
   * Karşılığı: iyzico submerchant, Stripe Connect account.
   */
  async createSubmerchant(_request: SubmerchantCreateRequest): Promise<SubmerchantResponse> {
    throw new Error('Submerchants not supported by this provider');
  }

  /** Alt-satıcı güncelle. Override edilmezse desteklenmez. */
  async updateSubmerchant(_request: SubmerchantUpdateRequest): Promise<SubmerchantResponse> {
    throw new Error('Submerchants not supported by this provider');
  }

  /** Alt-satıcıyı `externalId` ile sorgula. Override edilmezse desteklenmez. */
  async retrieveSubmerchant(_externalId: string): Promise<SubmerchantResponse> {
    throw new Error('Submerchants not supported by this provider');
  }
