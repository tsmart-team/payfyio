...
export function verifyWebhook(
  ...
  switch (provider) {
    ...
    case 'iyzico':
      return normalizeIyzico(raw, secret);
    default:
...
/**
 * iyzico posts notifications as a JSON body whose `signature` field is computed
 * over `iyziEventType + iyziReferenceCode` (not the raw bytes, not a header).
 * We parse first, then verify from those fields.
 */
function normalizeIyzico(raw: string, secret: string): PayfyioEvent {
  const body = parseJson(raw);
  const eventType: string = body?.iyziEventType ?? body?.iyziEventTye ?? '';
  const referenceCode: string = body?.iyziReferenceCode ?? '';
  const signature: string = body?.signature ?? '';

  if (!verifyIyzicoWebhookSignature(eventType, referenceCode, signature, secret)) {
    throw new WebhookSignatureError('Invalid iyzico webhook signature');
  }

  const paymentId: string = String(body?.paymentId ?? body?.paymentConversationId ?? referenceCode ?? '');
  const upper = eventType.toUpperCase();

  if (upper.includes('REFUND')) {
    return { type: 'refund.completed', provider: 'iyzico', paymentId, raw };
  }
  if (upper.includes('SUBSCRIPTION') || upper.includes('RECURRING')) {
    return mapIyzicoEventStatus(body?.subscriptionStatus ?? eventType) === 'cancelled'
      ? { type: 'subscription.canceled', provider: 'iyzico', subscriptionId: referenceCode, raw }
      : { type: 'subscription.renewed', provider: 'iyzico', subscriptionId: referenceCode, raw };
  }
  if (upper.includes('PAYOUT') || upper.includes('SETTLEMENT')) {
    return mapIyzicoEventStatus(eventType) === 'failed'
      ? { type: 'payout.failed', provider: 'iyzico', payoutId: referenceCode, raw }
      : { type: 'payout.paid', provider: 'iyzico', payoutId: referenceCode, raw };
  }

  // Default: a charge-lifecycle event (CHARGE_SUCCESS / *_FAILURE / *_INIT, etc.)
  return statusToEvent('iyzico', paymentId, mapIyzicoEventStatus(body?.status ?? eventType), raw);
}
