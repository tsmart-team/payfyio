...
describe('Garanti provider', () => {
  ...
  it('adds the recurring plan fields without disturbing the signed hash', async () => {
    const p = new Garanti(garantiCfg);
    const base = {
      price: '10',
      currency: 'TRY',
      conversationId: 'ORDER-R1',
      callbackUrl: 'https://x/cb',
      paymentCard: { cardNumber: '4', expireMonth: '12', expireYear: '2030', cvc: '123', cardHolderName: 'X' },
      buyer: { email: 'a@b.c', ip: '1.1.1.1' },
      basketId: 'b',
    ...
    const plain = await p.initThreeDSPayment(base);
    const rec = await p.initRecurringPayment({
      ...base,
      recurring: { totalPayments: 12, frequency: 'monthly', startDate: '2026-09-01' },
    });

    expect(rec.status).toBe('pending');
    expect(rec.threeDSHtmlContent).toContain('name="recurringtype" value="R"');
    expect(rec.threeDSHtmlContent).toContain('name="totalpaymentnum" value="12"');
    expect(rec.threeDSHtmlContent).toContain('name="frequencytype" value="M"');
    expect(rec.threeDSHtmlContent).toContain('name="frequencyinterval" value="1"');
    expect(rec.threeDSHtmlContent).toContain('name="startdate" value="20260901"');

    // The bank's signed field list is fixed and excludes the plan fields — if
    // this drifts, every recurring setup starts failing hash validation.
    const hashOf = (html: string) => html.match(/name="secure3dhash" value="([^"]+)"/)![1];
    expect(hashOf(rec.threeDSHtmlContent!)).toBe(hashOf(plain.threeDSHtmlContent!));
  });

  it('rejects a recurring plan with a nonsense repeat count', async () => {
    const p = new Garanti(garantiCfg);
    const res = await p.initRecurringPayment({
      price: '10',
      currency: 'TRY',
      callbackUrl: 'https://x/cb',
      paymentCard: { cardNumber: '4', expireMonth: '12', expireYear: '2030', cvc: '123', cardHolderName: 'X' },
      buyer: { email: 'a@b.c', ip: '1.1.1.1' },
      basketId: 'b',
      recurring: { totalPayments: 0, frequency: 'monthly' },
    } as any);
    expect(res.status).toBe('failure');
    expect(res.errorMessage).toMatch(/totalPayments/);
  });

  it('providers without recurring support refuse rather than charge once', async () => {
    const { Isbank } = await import('../../../../src/providers/isbank');
    await expect(
      new Isbank(isbankCfg).initRecurringPayment({ recurring: { totalPayments: 12, frequency: 'monthly' } } as any),
