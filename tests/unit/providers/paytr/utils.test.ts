...
describe('PayTR Utils', () => {
  ...
  describe('generatePayTRHash', () => {
    ...
    it('should produce consistent results for same inputs', () => {
      const args = [MERCHANT_ID, '1.2.3.4', 'ORD-1', 'a@b.com', '5000', '[]', '0', '0', 'TL', '0', MERCHANT_SALT, MERCHANT_KEY] as const;
      expect(generatePayTRHash(...args)).toBe(generatePayTRHash(...args));
    });

    it('should produce different results for different inputs', () => {
      const h1 = generatePayTRHash(MERCHANT_ID, '1.1.1.1', 'ORD-1', 'a@b.com', '5000', '[]', '0', '0', 'TL', '0', MERCHANT_SALT, MERCHANT_KEY);
      const h2 = generatePayTRHash(MERCHANT_ID, '2.2.2.2', 'ORD-2', 'a@b.com', '5000', '[]', '0', '0', 'TL', '0', MERCHANT_SALT, MERCHANT_KEY);
      expect(h1).not.toBe(h2);
    });

    it('uses merchant_key (not salt) as the HMAC key', () => {
      const args = [MERCHANT_ID, '1.1.1.1', 'ORD-1', 'a@b.com', '5000', '[]', '0', '0', 'TL', '0', MERCHANT_SALT] as const;
      expect(generatePayTRHash(...args, MERCHANT_KEY)).not.toBe(generatePayTRHash(...args, MERCHANT_SALT));
    });
  });
  ...
  describe('verifyPayTRCallback', () => {
    it('should return true for a valid hash', () => {
      ...
      expect(verifyPayTRCallback(merchantOid, MERCHANT_SALT, status, totalAmount, hash, MERCHANT_KEY)).toBe(true);
    });

    it('should return false for a tampered hash', () => {
      expect(verifyPayTRCallback('ORD-1', MERCHANT_SALT, 'success', '10000', 'INVALID_HASH==', MERCHANT_KEY)).toBe(false);
    });
    ...
    it('should return false for wrong status', () => {
      ...
      const hash = crypto
        ...
        .digest('base64');
      expect(verifyPayTRCallback(merchantOid, MERCHANT_SALT, 'failed', '10000', hash, MERCHANT_KEY)).toBe(false);
    });

    it('should reject a callback signed with the salt instead of the key (old buggy scheme)', () => {
      const merchantOid = 'ORD-1';
      const forged = crypto
        .createHmac('sha256', MERCHANT_SALT)
        .update(`${merchantOid}${MERCHANT_SALT}success10000`)
        .digest('base64');
      expect(verifyPayTRCallback(merchantOid, MERCHANT_SALT, 'success', '10000', forged, MERCHANT_KEY)).toBe(false);
    });
  });
  ...
  describe('generatePayTRToken', () => {
    it('should produce a base64 string', () => {
      const token = generatePayTRToken(MERCHANT_ID, 'ORD-001', '10000', MERCHANT_SALT, MERCHANT_KEY);
      expect(token).toMatch(/^[A-Za-z0-9+/]+=*$/);
    });

    it('should be deterministic', () => {
      const t1 = generatePayTRToken(MERCHANT_ID, 'ORD-1', '5000', MERCHANT_SALT, MERCHANT_KEY);
      const t2 = generatePayTRToken(MERCHANT_ID, 'ORD-1', '5000', MERCHANT_SALT, MERCHANT_KEY);
      expect(t1).toBe(t2);
    });
  });
  ...
  describe('generateBinDetailToken', () => {
    it('should produce a base64 string', () => {
      const token = generateBinDetailToken(MERCHANT_ID, '454360', MERCHANT_SALT, MERCHANT_KEY);
      expect(token).toMatch(/^[A-Za-z0-9+/]+=*$/);
    });

    it('should differ for different BIN numbers', () => {
      const t1 = generateBinDetailToken(MERCHANT_ID, '454360', MERCHANT_SALT, MERCHANT_KEY);
      const t2 = generateBinDetailToken(MERCHANT_ID, '411111', MERCHANT_SALT, MERCHANT_KEY);
